<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center">
      <h2 class="text-xl font-bold text-gray-700">Consumable Categories</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4">
      <input v-model="search" @input="fetchCategories" type="text" placeholder="Search categories..."
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-else-if="categories.length === 0"><td colspan="6" class="text-center py-8 text-gray-400">No categories found</td></tr>
          <tr v-for="cat in categories" :key="cat.id" class="border-b hover:bg-gray-50 transition">
            ...
            <td class="px-4 py-3 text-gray-400 text-xs">{{ cat.slug }}</td>
    ...
    <!-- Modal -->
    <AppModal :show="showModal" :title="editingCat ? 'Edit Category' : 'Add Category'" @close="closeModal">
    ...
    <!-- Delete Confirm -->
    <AppModal :show="showConfirm" title="Delete Category" max-width="max-w-sm" @close="showConfirm = false">
...
async function fetchCategories(page = 1) {
  loading.value = true
  try {
    const res = await api.get('/categories', { params: { page, per_page: 15, search: search.value } })
...
function openModal(cat = null) {
  ...
  form.value = cat ? { name: cat.name, description: cat.description || '', is_active: cat.is_active }
...
async function saveCategory() {
  ...
  try {
    if (editingCat.value) { await api.put(`/categories/${editingCat.value.id}`, form.value) }
...
async function confirmDelete() {
  try {
    await api.delete(`/categories/${confirmDeleteId.value}`)
