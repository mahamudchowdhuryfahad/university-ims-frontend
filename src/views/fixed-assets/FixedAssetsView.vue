<template>
  <div class="space-y-4" @click="showProductDropdown = false">
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-for="asset in assets" :key="asset.id" class="border-b hover:bg-gray-50 transition">
            <td class="px-4 py-3">
              <span class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs font-mono">{{ asset.asset_tag }}</span>
            </td>
            <td class="px-4 py-3">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" @click.stop>
        <h3 class="text-lg font-bold text-gray-700 mb-4">{{ editingAsset ? 'Edit Asset' : 'Add Fixed Asset' }}</h3>
  ...
  <input v-model="form.name" required
    class="w-full border rounded-lg px-3 py-2 text-sm mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
    placeholder="e.g. Dell Laptop, Steel Chair" />
</div>
            <div>
  ...
  <input v-model="form.serial_number"
    ...
    <div v-if="showAssignModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black bg-opacity-40" @click="showAssignModal = false"></div>
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
        <h3 class="text-lg font-bold text-gray-700 mb-1">Assign Asset</h3>
        ...
        <form @submit.prevent="assignAsset" class="space-y-3">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <button type="submit" :disabled="saving" class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg text-sm transition disabled:opacity-50">{{ saving ? 'Assigning...' : 'Assign' }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <!-- Dispose Modal -->
    <div v-if="showDisposeModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black bg-opacity-40" @click="showDisposeModal = false"></div>
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
        <h3 class="text-lg font-bold text-gray-700 mb-1">Dispose Asset</h3>
        <p class="text-sm text-gray-500 mb-4">{{ selectedAsset?.name }} ({{ selectedAsset?.asset_tag }})</p>
    ...
    <div v-if="showHistoryModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black bg-opacity-40" @click="showHistoryModal = false"></div>
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-bold text-gray-700">Asset History</h3>
          <button @click="showHistoryModal = false" class="text-gray-400 hover:text-gray-600 text-xl">×</button>
        </div>
        <div v-if="historyData" class="space-y-4">
          ...
          <div>
            <h4 class="text-sm font-semibold text-gray-600 mb-2">Transfers</h4>
            <div v-if="!historyData.transfers?.length" class="text-sm text-gray-400">No transfers</div>
            <div v-for="t in historyData.transfers" :key="t.id" class="py-2 border-b text-sm">
              <p class="text-gray-700">{{ t.from_department?.name || '—' }} → {{ t.to_department?.name || '—' }}</p>
              <p class="text-xs text-gray-400">{{ t.transfer_date }}</p>
            </div>
          </div>
          <div>
    ...
    <div v-if="showConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm text-center">
        ...
        <h3 class="text-lg font-bold text-gray-700 mb-2">Delete Asset</h3>
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter(p =>
    ...
    (p.sku && p.sku.toLowerCase().includes(productSearch.value.toLowerCase()))
  )
})
...
const disposeForm = ref({
  ...
  method: 'written_off', disposal_value: 0, reason: '',
})
...
function selectProduct(p) {
  ...
  form.value.serial_number = p.sku || ''
  form.value.brand_id = p.brand?.id || ''
}

function openModal(asset = null) {
  fetchProducts()
  productSearch.value = asset ? asset.name : ''
  showProductDropdown.value = false
