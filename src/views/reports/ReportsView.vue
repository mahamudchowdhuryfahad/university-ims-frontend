    ...
    <div v-if="activeTab === 'fixed-assets'" class="space-y-4">
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 flex flex-wrap gap-3 items-end">
        ...
        <div class="flex gap-2 ml-auto">
          <button @click="exportReport('excel')" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm transition flex items-center gap-1">
            📊 Export Excel
          </button>
          <button @click="exportReport('pdf')" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm transition flex items-center gap-1">
            📄 Export PDF
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 text-center">
          <p class="text-2xl font-bold text-purple-600">৳{{ formatNumber(faSummary.total_cost || 0) }}</p>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-for="(row, i) in faByDept" :key="i" class="border-b hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-gray-700">৳{{ formatNumber(row.total_cost) }}</td>
            </tr>
            <tr v-if="faByDept.length > 0" class="bg-blue-50 font-semibold">
              ...
              <td class="px-4 py-3">৳{{ formatNumber(faByDept.reduce((s, r) => s + Number(r.total_cost || 0), 0)) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
          ...
          <tbody>
            ...
            <tr v-for="(row, i) in faByRoom" :key="i" class="border-b hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-xs text-gray-500">
                <span v-for="(count, cat) in row.categories" :key="cat" class="mr-2">
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-if="faByCategory.length > 0" class="bg-blue-50 font-semibold">
              ...
              <td class="px-4 py-3">৳{{ formatNumber(faByCategory.reduce((s, r) => s + Number(r.total_cost || 0), 0)) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
