<template>
  <div class="space-y-4">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4">
      <input v-model="search" @input="fetchSchools"
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-else-if="schools.length === 0">
            <td colspan="7" class="text-center py-8 text-gray-400">No schools found</td>
          </tr>
            ...
            <td class="px-4 py-3">
              <div class="flex gap-2">
                <button @click="openModal(school)"
                  class="bg-blue-100 text-blue-600 hover:bg-blue-200 px-3 py-1 rounded-lg text-xs font-medium transition">✏️ Edit</button>
                <button @click="deleteSchool(school.id)"
                  class="bg-red-100 text-red-500 hover:bg-red-200 px-3 py-1 rounded-lg text-xs font-medium transition">🗑️ Delete</button>
              </div>
            </td>
          </tr>
        </tbody>
  ...
  <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg">
        <h3 class="text-lg font-bold text-gray-700 mb-4">{{ editingSchool ? 'Edit School' : 'Add School' }}</h3>
        ...
        <form @submit.prevent="saveSchool" class="space-y-3">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <button type="submit" :disabled="saving"
              class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg text-sm transition disabled:opacity-50">
              {{ saving ? 'Saving...' : 'Save' }}
            </button>
          </div>
        </form>
    ...
    <div v-if="showConfirm"
      ...
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm text-center">
        ...
        <h3 class="text-lg font-bold text-gray-700 mb-2">Delete School</h3>
