<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center">
      <h2 class="text-xl font-bold text-gray-700">Warehouses</h2>
      <button @click="openModal()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm transition">+ Add Warehouse</button>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-else-if="warehouses.length === 0"><td colspan="7" class="text-center py-8 text-gray-400">No warehouses found</td></tr>
          <tr v-for="w in warehouses" :key="w.id" class="border-b hover:bg-gray-50 transition">
            <td class="px-4 py-3 text-gray-400">{{ w.id }}</td>
            <td class="px-4 py-3 font-medium text-gray-700">{{ w.name }}</td>
            <td class="px-4 py-3"><span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-medium">{{ w.code }}</span></td>
            <td class="px-4 py-3 text-gray-500">{{ w.city || '—' }}</td>
            <td class="px-4 py-3 text-gray-500">{{ w.phone || '—' }}</td>
            <td class="px-4 py-3">
              <span :class="w.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'" class="px-2 py-0.5 rounded-full text-xs font-medium">{{ w.is_active ? 'Active' : 'Inactive' }}</span>
            </td>
            <td class="px-4 py-3">
              <div class="flex gap-2">
                <button @click="openModal(w)" class="bg-blue-100 text-blue-600 hover:bg-blue-200 px-3 py-1 rounded-lg text-xs font-medium transition">✏️ Edit</button>
                <button @click="deleteWarehouse(w.id)" class="bg-red-100 text-red-500 hover:bg-red-200 px-3 py-1 rounded-lg text-xs font-medium transition">🗑️ Delete</button>
              </div>
            </td>
          </tr>
        </tbody>
      ...
      <div class="px-4 py-3 flex justify-between items-center border-t text-sm text-gray-500">
        <span>Total: {{ pagination.total }} warehouses</span>
        <div class="flex gap-2">
    ...
    <!-- Modal -->
    <AppModal :show="showModal" :title="editingWarehouse ? 'Edit Warehouse' : 'Add Warehouse'" @close="closeModal">
      <div v-if="formError" class="bg-red-50 text-red-600 px-4 py-2 rounded-lg mb-4 text-sm">{{ formError }}</div>
      <form @submit.prevent="saveWarehouse" class="space-y-3">
        <div class="grid grid-cols-2 gap-3">
          <div class="col-span-2">
            <label class="text-xs font-medium text-gray-600">Warehouse Name *</label>
    ...
    <!-- Delete Confirm -->
    <AppModal :show="showConfirm" title="Delete Warehouse" max-width="max-w-sm" @close="showConfirm = false">
...
const form = ref({ name: '', code: '', phone: '', email: '', city: '', address: '', is_active: true })

async function fetchWarehouses(page = 1) {
  loading.value = true
  try {
    const res = await api.get('/warehouses', { params: { page, per_page: 15 } })
    warehouses.value = res.data.data.data
...
function openModal(w = null) {
  ...
  form.value = w ? { name: w.name, code: w.code, phone: w.phone || '', email: w.email || '', city: w.city || '', address: w.address || '', is_active: w.is_active }
    : { name: '', code: '', phone: '', email: '', city: '', address: '', is_active: true }
  showModal.value = true
}

function closeModal() { showModal.value = false; editingWarehouse.value = null }

async function saveWarehouse() {
  saving.value = true; formError.value = null
  try {
    if (editingWarehouse.value) { await api.put(`/warehouses/${editingWarehouse.value.id}`, form.value) }
    else { await api.post('/warehouses', form.value) }
    closeModal(); fetchWarehouses()
